...
description: Use this skill whenever the agent needs to identify or describe meteorological phenomena visible in a sky or outdoor photograph. Triggers include classifying the weather in an image, extracting diagnostic visual features of clouds or precipitation, estimating hail size from a scale-referenced frame, judging whether a photographed scene is consistent with radar or satellite context, routing a weather-photo submission in the VYou ingestion pipeline, or composing a cited evidence brief about a community-submitted weather report. Provides the authoritative phenomenon taxonomy (14 scored labels grouped across cloud morphology, non-severe precipitation, severe convection, and storm structures), per-phenomenon diagnostic feature vocabularies, the three-level confidence rubric, hail-size anchor-object guidance, and the out-of-scope escape hatch for non-weather images. Do NOT use for satellite-product interpretation, radar composite reading, or general weather forecasting — those live in the radar-and-satellite-reference skill.
